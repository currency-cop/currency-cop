...
import {
  ...
  POE_LOGIN_STEAM_URL,
  POE_MY_ACCOUNT_URL,
  POE_GET_CHARACTERS_URL,
  POE_STASH_ITEMS_URL,
  POE_LEAGUE_LIST_URL,
  POE_COOKIE_NAME,
  POE_COOKIE_REGEXP,
  POE_ACCOUNT_NAME_REGEXP,
  NINJA_CURRENCY_URL,
  NINJA_ITEM_URL,
  NINJA_CURRENCY_OVERVIEWS,
  NINJA_ITEM_OVERVIEWS
} from '../constants';
...
class CC {
  ...
  static Portfolios = new DataFile(
    'Portfolio',
    portfolioFilename,
    CC.Logger,
    []
...
class App extends React.Component {
  ...
  async load(skipAuthorization) {
    ...
    try {
      ...
      leagues = leagues.filter(
        league => Date.now() > +new Date(league.startAt)
      );
      ...
      if (CC.checkRateLimitWindows()) {
        ...
        let interval = setInterval(() => {
          this.setLoadingMessage(
            `GGG has rate limited your account for ${secondsLeft} seconds`
          );
          secondsLeft -= 1;
        }, 1000);
  ...
  doOfflineCheck() {
    ...
    let interval = setInterval(() => {
      this.setLoadingMessage(
        `No internet connection... trying again in ${secondsLeft} seconds.`
      );
      secondsLeft -= 1;
    }, 1000);
  ...
  deletePortfolio(portfolio) {
    ...
    return this.savePortfolios(portfolios).then(() => this.manageTabJobs());
  }
  ...
  async getPricesForEachLeague() {
    ...
    for (const { id: league } of leagues) {
      const prices = await Promise.all(
        Object.keys(ItemRateTypes).map(type => {
          this.setLoadingMessage(
            `Gathering Prices & Tabs (${league} - ${type})`
          );
  ...
  componentWillMount() {
    ...
    CC.Events.on('/screen/dashboard', () => {
      CC.Events.emit('/screen', {
        ...
        portfolioId: null,
        screen: <AppDashboard portfolios={this.state.portfolios} />
      });
    });
    ...
    CC.Events.on('/screen/portfolio', ({ portfolioId }) => {
      ...
      CC.Events.emit('/screen', {
        screenAction: `/screen/portfolio/${slug(portfolio.name, {
          lower: true
        })}`,
        portfolioId: portfolio.id,
        screen: <AppPortfolio portfolio={portfolio} />
      });
    });
    ...
    CC.Events.on('/screen/portfolio/update', ({ portfolioId }) => {
      ...
      if (!portfolio) {
        CC.EventLog.error(`Unable to find portfolio by id: ${portfolioId}`);
        CC.exception(
          `CC.Events.on('/screen/portfolio/update')`,
          new Error(`Unable to find portfolio by id: ${portfolioId}`)
        );
        return;
      }
