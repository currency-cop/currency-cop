  ...
  return new Promise((resolve, reject) => {
    ipcRenderer.once(options.onSuccess, (event, response) => {
      if (response && response.status > 200) {
        ...
        // that is not 200
        CC.ApiLog.warn(
          `${response.status} status response for [${
            options.url
    ...
    ipcRenderer.once(options.onError, (event, error) => {
      if (error.response) {
        ...
        CC.ApiLog.warn(
          `${error.response.status} status response for [${
            options.url
        ...
        CC.ApiLog.error(
          `Request sent to [${options.url}], no response received: ${error.message}`
        );
      } else {
        // Something happened in setting up the request that triggered an Error
        CC.ApiLog.error(
          `Unable to make HTTP request to [${options.url}]: ${error.message}`
        );
      }
...
function GetStashTabs(cookie, options) {
  ...
  return GetLeagueStashTab(cookie, options).then(response => {
    ...
    if (!response.data || !response.data.tabs) return [];
...
const GetDivCardOverview = (league, date) =>
  GetNinjaItem('DivinationCard', league, date);
