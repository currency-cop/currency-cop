class Item {
  ...
  property(name) {
    return this.source.properties
      ? this.source.properties.find(p => p.name === name)
      : null;
  }
  ...
  get orderId() {
    return (
      this._orderId ||
      (this._orderId = this.source.poeTradeId || this.source.id)
    );
  }
  ...
  get type() {
    return (
      this._type || (this._type = this.source.typeLine.replace('Superior ', ''))
    );
  }
  ...
  get quality() {
    ...
    return (
      this._quality ||
      (this._quality = property ? parseInt(property.values[0][0], 10) : 0)
    );
  }
