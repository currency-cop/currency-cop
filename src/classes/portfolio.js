import { UUID, formatNumber, getPercentageChange } from '../helpers';
...
class Portfolio {
  constructor({ id, name, league, history, tabs, tabItems, createdAt }) {
    this.id = id || UUID();
    this.name = name;
    this.tabs = tabs || [];
    this.league = league;
  ...
  buildReportItemChild(item) {
    return [
      item.stackSize,
      [item.source.inventoryId, item.source.w, item.source.x, item.source.y]
    ];
  }
  ...
  isItemPriceObject(item, priceItem) {
    ...
    if (priceItem.gemQuality || priceItem.gemLevel) {
      ...
      let levelTolerance = Math.max(
        0,
        Math.ceil(5 - Math.max(item.level, priceItem.gemLevel) * 0.25)
      );
      ...
      // Handle Special Gems
      let isSpecialSupport =
        item.fullName.indexOf('Enhance') > -1 ||
      ...
      let levelsClose =
        Math.abs(priceItem.gemLevel - item.level) <= levelTolerance;
      let qualityClose =
        Math.abs(priceItem.gemQuality - item.quality) <= qualityTolerance;
    ...
    if (priceItem.quality && priceItem.level) {
      return (
        priceItem.quality === item.quality && priceItem.level === item.level
      );
    }
  ...
  getChange(currency) {
    ...
    let previousHoldings =
      history.length > 1 ? this.history[this.history.length - 2].total : 0;
    let currentHoldings =
      history.length > 0 ? this.history[this.history.length - 1].total : 0;
    ...
    return {
      ...
      directionIndicator: !percentageChange.direction
        ? ''
        : percentageChange.direction === 'up'
        ? '+'
        : '-',
      directionClassName: percentageChange.direction
        ? percentageChange.direction
        : '',
      value: Math.abs(previousHoldings - currentHoldings),
      valueFormatted: formatNumber(
        Math.abs(previousHoldings - currentHoldings)
      ),
